...
10. **Markdown companion parity** — Markdown mirrors the HTML narrative structure and preserves resolver-backed links for FAQ, glossary, HowTo, People, Organizations, SoftwareApplication, source/document, and media entities. **The Markdown companion is NOT part of the DAV upload set.** When a collection is published to URIBurner, upload `.html`, `.ttl` and `.jsonld` only; the `.md` stays in the local generating tree (`{LLM_ROOT}/<Model Directory>/md/`) unless the user explicitly instructs otherwise, and such an override is per-request rather than a new default. Corollary: nothing may then carry a public DAV IRI naming the `.md` — drop any `rdfs:seeAlso`/`schema:sameAs` in the companion TTL pointing at `{DAV}/{stem}.md` (that one triple also propagates into the embedded `kgData` as a node plus an edge, and into the JSON-LD), plus any DAV `.md` URL in the Markdown's own companion-files list and in the HTML attribution footer. The document's own relative `../md/{stem}.md` links are correct and stay. Verify: `grep -c 'daas/{stem}[.]md'` returns 0 across `.html`/`.ttl`/`.jsonld`/`.md`, and `GET {DAV}/{stem}.md` returns 404. See `agent-rdf-memory/howto/remote-webdav-upload.ttl#step-markdownExcludedByDefault` (preferences.ttl Step 259).
11. **Authority denotation rules** — Person entities use profile URL `#this` IRIs using the same priority as `kg-generator`; SoftwareApplication and Country entities use DBpedia/Wikidata-centered IRIs where confidently available; DefinedTerm/skos:Concept glossary entries MUST use the confirmed DBpedia IRI as the **primary subject** (not a document-local alias with `owl:sameAs dbr:X`) whenever a confident DBpedia resource exists — `owl:sameAs` is reserved for cross-vocabulary alignment (DBpedia ↔ Wikidata) only. When no DBpedia or Wikidata IRI is confirmed, use a document-local hash IRI. The anti-pattern `:localConcept owl:sameAs dbr:X` is prohibited when the entity IS `dbr:X`.
...
13. **SPARQL query presentation** — when the RDF or source content contains `schema:SoftwareSourceCode` SPARQL examples, endpoint demos, or query recipes, render them as readable accordions with resolver-backed query-entity links, fenced/preformatted query text, a visible endpoint/service link, and a correctly URL-encoded live query link when an endpoint is known. Markdown companions must include the same query headings, resolver links, live links, and fenced `sparql` code blocks. **Every published SELECT MUST project at least one IRI-valued variable alongside its labels** — Virtuoso's `text/x-html+tr` rendering hyperlinks IRI cells and leaves literal cells as plain text, so a literals-only solution produces a results page with nothing to click and no way to continue exploring. Ask what entity each row is *about* and select that entity's IRI in addition to (never instead of) its `schema:name`; where a row joins two entities, project both. Name the variable after its entity with an `Iri` suffix (`?riskIri ?risk`, `?claimIri ?claim`, `?stepIri ?step`) and place it immediately before its own label column. For `GROUP BY` queries reintroduce the identifier with `SAMPLE(?entityIri)` or `GROUP_CONCAT` the IRIs into a second column. `DESCRIBE`/`CONSTRUCT` are exempt — they emit triples, so IRIs are inherent. Verify by executing each recipe with `format=application/sparql-results+json` and asserting at least one binding carries `"type": "uri"`; note that an anti-join deliberately returning zero rows has no bindings to inspect, so read its SELECT clause by hand rather than "fixing" a correct query. See `agent-rdf-memory/howto/sparql-example-query-iri-in-solution.ttl` (preferences.ttl Step 258).
...
16. **Dense exploration tools in collapsible containers** — when a KG Explorer or query workbench (SPARQL, SQL, or another query surface) is present alongside an editorial narrative, place each dense tool in a clearly labeled, keyboard-accessible native disclosure (`<details>/<summary>` or equivalent) that is closed by default, unless the user asks for it open or the selected artifact is explicitly an exploration-first dashboard. Keep the current-page anchor on the disclosure, preserve all controls and graph/query content inside its panel, provide visible focus and open/closed state, and ensure the summary explains what the tool does. This rule applies to the entire explorer/workbench section, separately from the KG Explorer controls tray, which also remains collapsed by default. At phone widths, the disclosure must remain usable and must not cause horizontal overflow.
17. **Canonical section sequence ordering** — generated HTML infographics and companion Markdown documents MUST follow the canonical section sequence: (1) Overview / Architecture / Comparison Matrix, (2) HowTo Execution Pipeline / Workflow, (3) Frequently Asked Questions (FAQ), (4) Core Technical Glossary, (5) Interactive Knowledge Graph Explorer (`#kg-explorer`), (6) SPARQL Workbench (`#sparql-workbench`), and (7) Attribution & Provenance Footer (`footer#attribution`). The Knowledge Graph Explorer and SPARQL Workbench MUST follow the Glossary — never place them before HowTo, FAQ, or Glossary.
...
When choosing among template styles, read `references/template-options.md`. It includes the compact harness reference plus user-supplied Claude templates for the Gartner dashboard, Semantic Medallion editorial/technical, competitive/head-to-head, Spec-Sheet Editorial Essay, Editorial Data-Viz, and Intelligence Network styles. The Spec-Sheet Editorial Essay template is for long-form narrative/thesis pieces and intentionally omits the KG Explorer and SPARQL workbench — see its entry in `references/template-options.md` before selecting it.

**Resolve every "preferred default" claim to its latest, non-superseded entry before generating.** `template-options.md` has chained multiple entries claiming default status for the same content class within a single day before (three entries, 2026-08-19–20, for Thesis & Framework content) — read the individual template section's own heading for a supersession banner, not just the top-of-file Selection Rule, and cross-check `preferences.ttl` for a later-numbered step on the same topic before citing any step by name or ID. See `agent-rdf-memory/howto/latest-standing-default-resolution.ttl`.

**Elicit the template choice only when it is genuinely ambiguous** — mirroring `kg-generator`'s RDF Format Elicitation / Generation Modality pattern:

- A named user request, a clear content-shape match (a comparison → competitive/head-to-head; Thesis & Framework content → the current standing default per the rule above), or an existing artifact being repaired all resolve without asking.
- Ask only when **two or more templates plausibly fit** and no standing default breaks the tie (e.g. a dense technical explainer that could go Gartner Dashboard or Semantic Medallion; a long-form thesis piece where it is unclear whether a sustained-narrative read or a data-exploration surface is wanted). Use a short closed question naming the candidates and the one-line tradeoff between them — see `template-options.md`'s Selection Rule for the exact phrasing pattern.
- If the same ambiguous shape is likely to recur for this user, record the elicited answer as a standing preference per `agent-rdf-memory/howto/elicit-before-conditional-default.ttl` step 3, so it is not re-asked indefinitely.

Also layer the **Editorial Enhancement Module** (`references/editorial-enhancement-module.md`) onto whichever template is selected, whenever the source has a quantitative claim worth foregrounding or a synthesis/cross-reference section connecting multiple meshed sources — this is not a template choice of its own, it augments any of the templates below.
...
- `scripts/rdf_infographic_harness.py` — optional reference helper functions for resolver URLs, query-type-specific SPARQL live URLs, a KG Explorer shell, footer attribution, and the footer "Explore Knowledge Graph using SPARQL" workbench.
  - **If you use any of those markup helpers, you must also emit `harness_styles()` into the page stylesheet.** No template in `assets/templates/` defines the `.attribution-*` / `.sparql-*` / `.entity-link` classes the helpers produce; without this CSS the attribution footer renders as full-bleed unstyled text and the SPARQL workbench as bare form controls with the live-query URL bleeding across the page. The failure is silent — valid HTML, no console error, correct content. `validate-harness-contract.py` now fails on it. Append the string to the existing stylesheet; it carries no `<style>` tags of its own, and nesting `<style>` inside `<style>` corrupts the first rule in the block.
  - Pass `include_markdown=False` to `attribution_footer()` on runs that produce RDF + HTML only, so the footer does not advertise a Markdown companion that was never written.
