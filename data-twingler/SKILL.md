...
| `judgmentModality` | `jev-shadow` (`none` \| `jev-shadow` \| `jev-active`) — see Judgment Modality |
...
**Default: `jev-shadow`.** Log System One Choice/Noul/Score beside the current
router decision; do **not** override until promoted to `jev-active`. Always
**fail open** to the existing Execution Routing path.
