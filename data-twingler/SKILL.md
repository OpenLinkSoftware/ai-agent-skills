...
| `judgmentModality` | `jev-active` (**CURRENT**, set 2026-09-20) (`none` \| `jev-shadow` \| `jev-active`) — see Judgment Modality |
...
**Default: `jev-active` (activated 2026-09-20).** Honor System One Choice/Noul/Score
when confidence/probability meets thresholds; otherwise fall back to the existing
router. Use `jev-shadow` to log advice without overriding. Always **fail open** to
the existing Execution Routing path.
...
- **Judgment modality:** default `jev-active`; never block on Jev availability; never pass secrets into System One state; never let Jev author SPARQL/SQL/GraphQL — only route over templates and rungs documented in this skill.
