...
## Workflow
1. Elicit, as separate required inputs: resource type (file/graph/api/chatservice), **pricing model (one-time purchase vs. recurring subscription — a distinct question from resource type; never infer it silently from how the price happens to be phrased)**, host, license IRI, price (plus billing interval/unit when the pricing model is subscription), description
...
6. Run Post-Generation Checklist, save to the output directory the user specifies for this session (ask if not already given — this is user/machine-specific, e.g. previously `/Users/jacqui/source/ttl/my_git_forks/virtuoso-82-offers-description/` for JacquiHand's machine, which won't exist on another contributor's machine), provide loading instructions
...
- duration (one-time purchase): If the price is a one-time, non-recurring charge, `opllic:hasDuration` MUST be the canonical `<http://data.openlinksw.com/oplweb/license/License-Duration#perpetual>` IRI — also never invent/type a local Duration node. Before using any Duration IRI not already listed in `references/offer-iri-patterns.md`, query the live store for existing `opllic:Duration` instances (`SELECT ?d ?name WHERE { GRAPH ?g { ?d a opllic:Duration . OPTIONAL { ?d schema:name ?name } } }`) rather than assuming one exists or minting a fresh one — added 2026-09-08 after confirming only `#ongoing-subscription` and `#annual` existed live at the time.
...
- license typing: Every License (File Access, Graph Access, or API Access — all three) MUST always be typed `opllic:ProductLicense, opllic:Product, opllic:ACLOnly`. `opllic:SubscriptionLicense` is a **fourth, conditional** type: include it if and only if the License's own Offer is typed `oplofr:SubscriptionOffer` (i.e., a genuinely recurring price — see the "subscription pricing" rule below). A one-time, non-recurring offer's License must NOT carry `opllic:SubscriptionLicense`. **Bug found and fixed 2026-09-10**: this rule previously said SubscriptionLicense was required "regardless of whether the price is one-time or recurring" — that was wrong, and it caused every one-time File Access offer generated under the old rule to be mistyped as a subscription license. That mistyping is also the confirmed root cause of a live ods-qa payment-completion bug: the shop's Stripe-SPT completion procedures branch on the License's `rdf:type` to decide which settlement path to use, so a one-time License wrongly carrying `SubscriptionLicense` gets routed into subscription-only settlement logic and fails to complete. The SHACL gate now enforces the corrected, conditional rule via a `sh:sparql` constraint on `BaseLicenseShape` comparing the Offer's `oplofr:SubscriptionOffer` typing against the License's `opllic:SubscriptionLicense` typing — the two must always agree.
...
- schema.org prefix name: the bound prefix for `http://schema.org/` MUST be `schema:` — never `schema1:`. Found 2026-09-08: the two live prior-art reference files (whatever process originally wrote them) use `schema1:`, an artifact of their own serialization, not a naming convention to imitate — SKILL.md's own templates above have always used `schema:`. Regenerating or editing an offer file that currently has `@prefix schema1:` should rename it to `schema:` throughout (a safe global find/replace, since `schema1` never appears as a substring of anything else in these files).
- language tags on prose literals: every natural-language string value on `schema:name`, `schema:description`, `schema:comment`, `skos:prefLabel`, `skos:altLabel`, `foaf:name`, and `cc:attributionName` MUST carry `@en` — e.g. `schema:name "Food Bookmark Collection (RDF-Turtle) — File Access on URIBurner"@en`. This applies everywhere those predicates appear with free-text prose: on the Offer, License, PriceSpecification, the shared Product/WebAPI blocks (already `@en`-tagged in prior art — keep as is), `acl:Authorization`'s `schema:name`, and `oplacl:ConditionalGroup`'s `foaf:name`. **Two required exceptions, both because the SHACL gate pins the datatype**: the `source:` document's own `schema:name` (`DocumentShape` requires `sh:datatype xsd:string`) and the `opllic:Duration` node's `schema:name` (`DurationShape` requires the same) must stay untagged plain/`xsd:string` literals — adding `@en` there gives the literal `rdf:langString` datatype instead of `xsd:string` and fails the gate. Never add a language tag to `schema:price` (`xsd:decimal`), `schema:priceCurrency`/`schema:unitCode`/`oplofr:interval`/`oplofr:offerNumber` (`xsd:string` codes, not prose), `oplofr:intervalCount` (`xsd:integer`), or any `xsd:dateTime` value — language tags belong on natural-language text, not codes, numbers, or dates.
