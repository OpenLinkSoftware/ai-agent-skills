...
`templates/upgrade.sql` is a single-file convenience wrapper — a concatenation of both templates above, automatic pre-flight backups, and one final, placeholder-parameterized `WEBLOG_DAV_DEPLOY_SKINNED` call — for handing to an operator who wants to apply the upgrade (or do a first-time deploy) manually via their own working `isql` session, without needing to locate and run three separate files in order or remember to back anything up first. Before Section 1 drops `WEBLOG_SUBSCRIBER`, its current contents (if any) are copied into a fresh, timestamped `WEBLOG_SUBSCRIBER_BACKUP_<timestamp>` table; before the REDEPLOY block overwrites `index.vsp`/`dashboard.html`, their current content (if any) is copied into `DB.DBA.WEBLOG_UPGRADE_BACKUP`. Neither backup table is ever dropped by a later run, and restore steps are documented in-line as comments right above each backup block. This makes "can I revert if it fails" true by default for what this file changes — it is not a substitute for backing up anything else on the target instance. It is a dated snapshot, not a maintained source of truth: regenerate it (concatenate the two source templates plus fresh backup/redeploy blocks) whenever either source template changes materially, rather than trusting a stale copy indefinitely.
