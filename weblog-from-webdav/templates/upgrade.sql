...
-- ============================================================================
-- PRE-FLIGHT BACKUP 1 of 2, part 2 -- RESTORE. Section 1 above just DROPped
-- and recreated WEBLOG_SUBSCRIBER (fresh and empty), after the backup step
-- near the top of this file archived whatever was in it. Every real
-- subscriber sat only in that timestamped backup table until this restore
-- step existed -- confirmed live 2026-09-24: re-running this file wiped a
-- genuinely confirmed subscriber out of the live table on every single
-- run, with no restore ever happening automatically, silently
-- accumulating orphaned WEBLOG_SUBSCRIBER_BACKUP_* tables instead (14 of
-- them from one day of repeated runs). This finds the most recently
-- created backup table (the one the pre-flight step just made, by
-- construction the same run) and inserts its rows straight back into the
-- freshly emptied live table. Best-effort and never fatal: a first-ever
-- install has no backup table to restore from, which is not an error.
create procedure DB.DBA.TMP_WEBLOG_UPGRADE_RESTORE_SUBSCRIBERS ()
{
  ...
  if (backup_table_name is null)
    return 'No WEBLOG_SUBSCRIBER_BACKUP_* table found -- nothing to restore (first-ever install).';
  exec (sprintf ('insert into DB.DBA.WEBLOG_SUBSCRIBER select * from %s', backup_table_name));
  return sprintf ('Restored WEBLOG_SUBSCRIBER from %s after Section 1 recreated it.', backup_table_name);
}
;
select DB.DBA.TMP_WEBLOG_UPGRADE_RESTORE_SUBSCRIBERS ();
drop procedure DB.DBA.TMP_WEBLOG_UPGRADE_RESTORE_SUBSCRIBERS;
